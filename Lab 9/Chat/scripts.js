...
var theHistory = function(Name, messageText, isDeleted, isEdited) {
	return {
		username: Name,
        delete: isDeleted,
        edit: isEdited,
...
function onAddButtonClick(){
    ...
    var task=theHistory(username, todoText.value,false, false);
    if(toWhomClick==null)
    {
             ...
             for(var i=0;i<appState.HistoryList.length;i++)
                 {
                     if(idTemp == appState.HistoryList[i].id)
                         {       
                             ...
                             var task=theHistory(username, appState.HistoryList[i].text,appState.HistoryList[i].delete, appState.HistoryList[i].edit);
